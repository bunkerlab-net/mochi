...
- Add a `mix` option to `/play` and `/playnow`: `mix:true` queues the resolved track followed by that track's YouTube mix (radio), filling up to the server's `playlist-limit`, so a dynamic playlist starts right away instead of when the queue runs dry. A mix comes from a single video, so a query that resolves several tracks keeps the first as the seed; non-YouTube tracks, a limit with no room, and videos without a mix all say so in the reply.
- Add an `autoplay` option to `/play` and `/playnow`: `autoplay:true|false` overrides the server's autoplay setting for the current session only. With no override set the server setting applies, and once you set one it sticks for the rest of the session, so a later `/play` that omits the option keeps it. The override is dropped when the session ends (`/stop`, or Mochi leaving an empty voice channel), while `/disconnect` keeps it along with the queue.

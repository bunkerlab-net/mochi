...
import type SpotifyAPI from "./spotify-api.js";
...
export default class {
  ...
  async getSongs(
    ...
    try {
      return await this.handleUrlSource(
        query,
        playlistLimit,
        shouldSplitChapters,
      );
  ...
  private async handleUrlSource(
    ...
    if (YOUTUBE_HOSTS.includes(url.host)) {
      ...
      const [convertedSongs, msg] = await this.handleSpotifySource(
        query,
        playlistLimit,
        shouldSplitChapters,
      );
  ...
  private async handleSpotifySource(
    ...
    return [convertedSongs, extraMsg];
  }

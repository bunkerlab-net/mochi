...
const isExecaError = (
  error: unknown,
): error is { stderr?: string; shortMessage?: string } =>
  typeof error === "object" &&
  error !== null &&
  ("stderr" in error || "shortMessage" in error);

const toYouTubeWatchUrl = (videoIdOrUrl: string) =>
...
const hasPathSeparator = (candidatePath: string) =>
  candidatePath.includes("/") || candidatePath.includes("\\");
